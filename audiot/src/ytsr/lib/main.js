...
const ytsr = async (searchString, options, rt = 3) => {
  ...
  if (!parsed.json) return ytsr(searchString, options, rt - 1);
...
ytsr.getFilters = async (searchString, options) => {
  ...
  if (!parsed.json) return ytsr.getFilters(searchString, options);
